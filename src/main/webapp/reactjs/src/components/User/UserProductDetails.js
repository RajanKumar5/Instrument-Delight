...
class UserProductDetails extends Component {
    ...
    componentDidMount() {
        ...
        if (productId) {
            axios.get("http://localhost:8080/getProduct/" + productId)
                .then(response => {
                    this.setState({
    ...
    placeOrder = () => {
        ...
        axios.post("http://localhost:8080/placeOrder/", order)
            .then(response => {
                if (response.data) {
    ...
    updateProductQuantity = (quantity) => {
        const product = {
            ...
            quantity: this.state.instrumentQuantity - quantity
        }
        axios.put("http://localhost:8080/home/updateQuantity/", product);
    }
                ...
                <center>
                    <Card style={{ width: '23rem' }} className="mb-5 shadowBox">
                        ...
                        <ListGroup className="list-group-flush">
                            <ListGroupItem>
                                <h5>M.R.P: <del className="text-muted">${Math.round(this.state.instrumentPrice * (1 + (new Date().getMonth() / 100)))}</del> ${this.state.instrumentPrice} </h5>
                            </ListGroupItem>
                            <ListGroupItem>
                                {this.state.instrumentQuantity <= 0 ?
                            ...
                            {this.state.instrumentQuantity <= 0 ? null :
                                <ListGroupItem>
                                    <ButtonGroup>
